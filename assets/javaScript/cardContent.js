...
function cardRecipe(list) {
  ...
  for (let i = 0; i < numberOfRecipe; i++) {
    ...
    let textBtn = document.createTextNode("Commander");
    //Ajoutr d'un event au bouton
    cardsButton.addEventListener("click", (e) => {
      shoppingCard.addDish(list[i], 1);
    });
    cardsButton.appendChild(textBtn);
...
class ShoppingCard {
  ...
  addDish(dishObject, qte) {
    this.dishList.push([dishObject, qte]);

    this.updateShoppingCard();
  }

  updateShoppingCard() {
    let shopingCard = document.querySelector("#shoppingList__content");
    let shopingCardTotal = document.querySelector("#shoppingCartTotal");
    let totalCart = 0;
    shopingCard.innerHTML = this.dishList
      .map((dish) => {
        let totalDishPrice =
          parseFloat(dish[0].prix.replace(",", ".")) *
          parseFloat(parseFloat(dish[1]));
        console.log(totalDishPrice);
        totalCart += totalDishPrice;
        return `
      <tr>
          <td>${dish[0].name}</td>
          <td>${dish[0].prix}</td>
          <td>${dish[1]}</td>
          <td>${totalDishPrice}</td>
        </tr>
      `;
      })
      .join("");

    shopingCardTotal.innerHTML = totalCart;
